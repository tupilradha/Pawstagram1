// src/pages/Timeline.jsx — includes Meds + Symptoms add/edit
import { useEffect, useState } from "react";
import { useDog } from "../context/DogContext";
import {
  getVetVisits, getVaccines, getMedications, getSymptoms,
  saveMedication, deleteMedication, saveSymptom, deleteSymptom,
} from "../db/db";
import { validateMedicationForm, validateSymptomForm } from "../utils/validate";
import FieldError from "../components/FieldError";

// ─── Timeline builder ─────────────────────────────────────
const DOT_COLORS = { vet: "#38bdf8", vaccine: "#a78bfa", medication: "#4ade80", symptom: "#fb923c" };
const SEV_COLOR  = { mild: "#4ade80", moderate: "#f59e0b", severe: "#f87171" };

function buildTimeline(dogId) {
  const events = [];
  getVetVisits(dogId).forEach(v => events.push({ id: v.id, date: v.date, type: "vet", icon: "🏥", title: v.reason || "Vet Visit", sub: [v.clinic, v.vet].filter(Boolean).join(" · "), notes: v.notes }));
  getVaccines(dogId).forEach(v => events.push({ id: v.id, date: v.dateGiven, type: "vaccine", icon: "💉", title: v.name || "Vaccine", sub: v.nextDueDate ? `Next due: ${v.nextDueDate}` : "" }));
  getMedications(dogId).forEach(m => events.push({ id: m.id, date: m.startDate, type: "medication", icon: "💊", title: m.name || "Medication", sub: [m.dosage, m.frequency].filter(Boolean).join(" · "), raw: m }));
  getSymptoms(dogId).forEach(s => events.push({ id: s.id, date: s.date, type: "symptom", icon: "📝", title: s.description || "Symptom", sub: s.severity ? `Severity: ${s.severity}` : "", severity: s.severity, raw: s }));
...
function exportJSON(dog, events) {
  const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), dog: { name: dog.name, breed: dog.breed, age: dog.age }, totalRecords: events.length, records: events }, null, 2)], { type: "application/json" });
...
export default function Timeline() {
  ...
  const [panel, setPanel]     = useState(null); // null | "med" | "sym"
  ...
  function refresh() { if (activeDogId) setEvents(buildTimeline(activeDogId)); }
  useEffect(() => { refresh(); }, [activeDogId]);
  ...
  function saveMed() {
    ...
    saveMedication({ ...medForm, dogId: activeDogId, ...(medId ? { id: medId } : {}) });
  ...
  function saveSym() {
    ...
    saveSymptom({ ...symForm, dogId: activeDogId, ...(symId ? { id: symId } : {}) });
    refresh(); setPanel(null);
  }
  ...
  const filtered = events.filter(e => {
    ...
    const matchQuery = !query || e.title.toLowerCase().includes(query.toLowerCase()) || (e.sub || "").toLowerCase().includes(query.toLowerCase()) || (e.notes || "").toLowerCase().includes(query.toLowerCase());
  ...
  return (
    <div className="page">
      <header className="page__header">
        ...
        {activeDog && <button className="tl-export-btn" onClick={() => exportJSON(activeDog, events)}>⬇ Export</button>}
      </header>
      ...
      {panel === "med" && (
        <div className="card">
          ...
          <div className="form">
            ...
            <input className={`form__input ${medErr.dosage ? "form__input--error" : ""}`} value={medForm.dosage} maxLength={50} onChange={e => setMedForm({ ...medForm, dosage: e.target.value })} placeholder="e.g. 250mg" />
            <FieldError msg={medErr.dosage} />
            ...
            <input className={`form__input ${medErr.endDate ? "form__input--error" : ""}`} type="date" value={medForm.endDate} min={medForm.startDate || today} onChange={e => setMedForm({ ...medForm, endDate: e.target.value })} />
      ...
      {panel === "sym" && (
        <div className="card">
          ...
          <div className="form">
            ...
            <textarea className={`form__textarea ${symErr.description ? "form__input--error" : ""}`} value={symForm.description} maxLength={300} onChange={e => setSymForm({ ...symForm, description: e.target.value })} placeholder="Describe the symptom..." rows={3} />
            ...
            <div className="radio-group">
              {["mild","moderate","severe"].map(s => (
                <label key={s} className={`radio-option ${symForm.severity === s ? "radio-option--active" : ""}`}>
                  <input type="radio" name="severity" value={s} checked={symForm.severity === s} onChange={() => setSymForm({ ...symForm, severity: s })} />
                  {SEV_LABELS[s]}
                </label>
              ))}
            </div>
      ...
      <div className="filter-chips">
        {TYPE_FILTERS.map(f => (
          <button key={f.key} className={`filter-chip ${filter === f.key ? "filter-chip--active" : ""}`}
      ...
      <div className="timeline">
        {Object.entries(grouped).map(([month, evts]) => (
          <div key={month} className="timeline__group">
            <div className="timeline__month-label">{formatMonth(month)}</div>
            {evts.map((e, i) => (
              <div key={e.id} className="timeline__item">
                <div className="timeline__line-wrap">
                  <div className="timeline__dot" style={{ background: DOT_COLORS[e.type] || "#888" }} />
                  {i < evts.length - 1 && <div className="timeline__line" />}
                </div>
                <div className="timeline__content">
                  <div className="timeline__header">
                    <span className="timeline__icon">{e.icon}</span>
                    <span className="timeline__title">{e.type === "symptom" ? (e.title.length > 40 ? e.title.slice(0,40)+"…" : e.title) : e.title}</span>
                  ...
                  {e.severity && <span className="badge" style={{ background: SEV_COLOR[e.severity]+"33", color: SEV_COLOR[e.severity], marginTop: 4, display:"inline-block" }}>{SEV_LABELS[e.severity]}</span>}
