...
export default function Home() {
  ...
  const globalAlerts = dogs.flatMap(d => {
    const alerts = dogAlerts[d.id] || [];
    return alerts
      .filter(a => a.type !== "green")
      .map(a => `${d.name}'s ${a.label}`);
  });
  ...
  return (
    <div className="home-dark">
      ...
      <div className="home-dark__header">
        <div>
          <h1 className="home-dark__title">My Dogs</h1>
          <p className="home-dark__date">{formatDate()}</p>
        </div>
        <button className="home-dark__add-btn" onClick={() => navigate("/profile/new")}
          aria-label="Add dog">
      ...
      {globalAlerts.length > 0 && (
        <div className="home-dark__alert">
          ...
          <div>
            <div className="home-dark__alert-title">
              {globalAlerts.length} thing{globalAlerts.length > 1 ? "s" : ""} need attention
            </div>
            <div className="home-dark__alert-sub">
              {globalAlerts.slice(0, 3).join(" · ")}
              {globalAlerts.length > 3 ? ` · +${globalAlerts.length - 3} more` : ""}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="home-dark__grid">
        {dogs.map((dog, i) => {
          ...
          return (
            <div
              key={dog.id}
              className={`home-dark__card ${isActive ? "home-dark__card--active" : ""}`}
              onClick={() => { setActiveDogId(dog.id); navigate("/vet-visits"); }}
            >
              {isActive && (
                <div className="home-dark__active-pill">
                  <span className="home-dark__active-dot" />
                  Active
                </div>
              )}

              <div className="home-dark__avatar"
              ...
              <div className="home-dark__badges">
                {alerts.map((a, j) => (
                  <span key={j} className={`home-dark__badge home-dark__badge--${a.type}`}>
                    {a.label}
                  </span>
                ))}
              </div>
